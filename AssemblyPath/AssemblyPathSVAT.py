...
class AssemblyPathSVA():
    ...
    def updateTheta(self, M_train):
        ...
        denom = np.sum(self.expTau*self.exp_square_lambda_matrix()*M_train,axis=1)*self.lengths*self.lengths  
        
        numer =  self.lengths*np.sum(self.X*self.eLambda*self.expTau*M_train,axis=1) 
    ...
    def updateTau(self, M_train):
        ...
        if self.bFixedTau:
            self.updateFixedTau(M_train)
        else:
    
            if self.bLogTau:
                self.updateLogTau(M_train)
            else:
                self.updateTauBeta(M_train)
    
    
    def updateFixedTau(self, M_train):
        square_diff_matrix = self.exp_square_diff_matrix()*M_train  

        betaTemp = self.beta + 0.5*np.sum(square_diff_matrix)
        mOmega = np.sum(M_train)
        alphaTemp = self.alpha + 0.5*mOmega

        tempTau = alphaTemp/betaTemp
        
        tempLogTau = digamma(alphaTemp) - np.log(betaTemp)

        self.betaTau.fill(betaTemp/mOmega)

        self.alphaTau.fill(alphaTemp/mOmega)
    ...
    def updateLogTau(self, M_train):
        ...
        mR = np.ma.masked_where(M_train==0, R)
        ...
        np.place(self.betaTau, M_train, mBetaTau)

        logExpTau1D = digamma(self.alpha + 0.5) - np.log(mBetaTau)
        ...
        np.place(self.expLogTau, M_train, yest_sm)
         
        np.place(self.expTau, M_train, np.exp(yest_sm))
    

    def updateTauBeta(self, M_train):
        ...
        logDiff1D = np.log(np.ma.compressed(mDiff))
        logX1D = np.log(np.ma.compressed(mR))
        try:
        ...
        mBetaTau = self.beta + 0.5*np.exp(yest_sm)
        
        np.place(self.betaTau, M_train, mBetaTau)

        np.place(self.expLogTau,M_train, digamma(self.alpha + 0.5) - np.log(mBetaTau))
        
        np.place(self.expTau,M_train, (self.alpha + 0.5)/(mBetaTau))
    ...
    def update(self, maxIter, removeRedundant,M_train,logFile=None,drop_strain=None,relax_path=False, uncertainFactor=None,minDiff=1.0e-3):
        ...
        diffElbo = 1.0
        currElbo=self.calc_elbo(M_train)
    ...
    def updateGammaFixed(self, maxIter, M_train, drop_strain=None, relax_path=False):
    ...
    def initNMFGamma(self,assCopyGamma,M_train):
    ...
    def mean_diff(self,M_train):
    
        diff_matrix = self.divF_matrix()*M_train
                        
        return np.sum(diff_matrix)/np.sum(M_train)
    
     
    def gene_mean_deviance(self, M_train):
        ...
        for gene in self.genes:
            ...
            for unitig in unitigs:
                v_idx = self.mapGeneIdx[gene][unitig]
                mean_dev += np.sum(deviance_matrix[v_idx,:]*M_train[v_idx,:])
                nComp += np.sum(M_train[v_idx,:])
            gene_means[gene] = mean_dev/nComp
    ...
    def calc_elbo(self,M_train):
        ...
        dTemp1 = (self.alpha + 0.5)*np.sum(np.log(self.betaTau)*M_train) - nTOmega*sps.gammaln(self.alpha + 0.5)
       
        dTemp2 = np.sum((self.alpha - 0.5)*self.expLogTau*M_train) + np.sum(self.betaTau*self.expTau*M_train)
    ...
    def filterHaplotypes(self,retained,gammaIter,M_train):
    ...
    def removeRedundant(self, minIntensity, gammaIter, relax_path, M_train, uncertainFactor=None):
        ...
        self.filterHaplotypes(retained,gammaIter,M_train)
