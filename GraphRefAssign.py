...
class Graph_Correct():
    ...
    def __init__(self,randomState, unitigGraph, theta = 1.0e-6, maxIter = 100, alphaPrior = 1., minP = 1.0, tau = 1.0):
        ...
        for unitig in self.unitigs:
            length = self.unitigGraph.lengths[unitig]
            adjLength = 0
            ...
            if nC == 0:
                ...
                adjLength = length - self.unitigGraph.overlapLength
                outLink = next (iter (self.unitigGraph.overlaps[unitig].values()))
                self.overlapEnd[unitig] = outLink[0][0]
            else:
                adjLength = length - 2*self.unitigGraph.overlapLength
                if adjLength < 0:
                    adjLength = 0
                nC = 2
            ...
            for read, aligns in self.pathMap.items():
                ...
                for align in aligns:
                    ...
                    if len(unitigs) == 1:
                        ...
                        if self.NC[unitigs[0]] == 0:
                            trimStart = False
                            trimEnd = False
                        
                        if self.NC[unitigs[0]] == 1:
                            end = self.overlapEnd[unitigs[0]]
                            mapDirn = align[0][0][1]
                            if mapDirn is False:
                                end = not end
                            trimEnd = end
                        ...
                        if trimStart:
                            if startPos < self.unitigGraph.overlapLength:
                                startPos = self.unitigGraph.overlapLength
                        
                        if trimEnd:
                            endMarg = self.unitigGraph.lengths[unitigs[0]] - self.unitigGraph.overlapLength
                            
                            if endPos > endMarg:
                                endPos = endMarg
                                     
                        if self.adjLengths[unitigs[0]] > 0:
                            fHits[0] = float(endPos - startPos)/float(self.adjLengths[unitigs[0]])        
                        else:
                            fHits[0] = 1.0
                        ...
                        if self.adjLengths[startUnitig] > 0:
                            #if connected at other end then trim start
                            if self.NC[startUnitig] > 1:
                                if startPos < self.unitigGraph.overlapLength:
                                    startPos = self.unitigGraph.overlapLength
                            #must be leaving on edge..
                            endMarg = self.unitigGraph.lengths[startUnitig] - self.unitigGraph.overlapLength
                            fHits[0] =  float(endMarg - startPos)/float(self.adjLengths[startUnitig])
                        
                        endUnitig = unitigs[-1]
                        if self.adjLengths[endUnitig] > 0:
                            #if connected at other end then trim end
                            if self.NC[endUnitig] > 1:
                                endMarg = self.unitigGraph.lengths[endUnitig] - self.unitigGraph.overlapLength
                            
                                if endPos > endMarg:
                                    endPos = endMarg
                            fHits[-1] =  float(endPos - self.unitigGraph.overlapLength)/float(self.adjLengths[endUnitig])
