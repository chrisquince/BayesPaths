...
class bnmf_vb:
    ...
    def initialise(self,init_UV='exp'):
        ...
        self.exp_tau = 1.0/(0.5 + self.R)
        self.exp_logtau = numpy.log(self.exp_tau)
    ...
    def elbo(self):
        ...
        # Log likelihood               
        total_elbo += - 0.5 * self.size_Omega * math.log(2*math.pi )  + 0.5 * numpy.sum(self.exp_logtau)\
                      - 0.5 * self.exp_square_diff()
    ...
    def exp_square_diff(self): 
        ...
        return (self.M * self.exp_tau * ( ( self.R - numpy.dot(self.exp_U,self.exp_V.T) )**2 + \
    ...
    def update_U(self,k):   
        ...
        self.tau_U[:,k] = (self.M * self.exp_tau *( self.var_V[:,k] + self.exp_V[:,k]**2 )).sum(axis=1) #sum over j, so rows
    ...
    def update_V(self,k):
        ...
        self.tau_V[:,k] = (self.exp_tau.T*self.M.T*( self.var_U[:,k] + self.exp_U[:,k]**2 )).T.sum(axis=0) #sum over i, so columns
        self.mu_V[:,k] = 1./self.tau_V[:,k] * (-lamb + (self.exp_tau.T*self.M.T * ( (self.R-numpy.dot(self.exp_U,self.exp_V.T)+numpy.outer(self.exp_U[:,k],self.exp_V[:,k])).T*self.exp_U[:,k] )).T.sum(axis=0)) 
    ...
    def log_likelihood(self):
        ...
        return - 0.5 * self.size_Omega *  math.log(2*math.pi) + 0.5*numpy.sum(self.exp_logtau) \
             -  0.5 * (self.M*self.exp_tau * ( self.R - numpy.dot(self.exp_U,self.exp_V.T))**2).sum()
