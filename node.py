...
class FacNode(Node):
    ...
    def __str__(self): 
        ...
        #count up no. of unobserved variables
        nObserved = 0
        observed = []
        observed_names = []
        dims = []
        ...
        slices = []
        idx = 0
        for var in self.nbrs:    
            if var.observed < 0:
                observed.append(var.nid)
                observed_names.append(var.name)
                dims.append(var.dim)
                slices.append(slice(0,var.dim,1))
            else:
                unobserved.append(idx)
                unobserved_vals.append(var.observed)
                slices.append(slice(var.observed,var.observed + 1,1))
            idx += 1
            
            
        retString = str(len(observed)) + "\n"
        
        retString += ' '.join([str(i) for i in observed]) + "\n"
        
        #retString += '#' + ' '.join([str(i) for i in observed_names]) + "\n"

        retString +=  ' '.join([str(i) for i in dims]) + "\n"
        ...
        nZeros = np.sum(Pred > 0.)
        
        retString += str(nZeros) + "\n"
        PredFlat = Pred.flatten('F')
        PredFlatNZ = PredFlat[PredFlat > 0.]
        
        pidx = 0
        psum = 0.0
        for val in PredFlat:
            if val > 0.:
                retString += str(pidx) + " " + str(val) + "\n"
                psum += val
            pidx += 1
        
        if psum < 1.0e-10:
            print("Error:" + str(psum))
        return retString 
